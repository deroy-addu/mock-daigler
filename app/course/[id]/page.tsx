...
function StatusPill({ status }: { status: string }) {
  ...
  const style = styleMap[status] || "bg-stone-100 text-stone-600";

  return (
    <span className={`px-3 py-1 rounded-full text-[10px] uppercase tracking-wider font-bold ${style}`}>
      {status}
    </span>
  );
}
...
export default function CourseDetailPage() {
  ...
  const courseAssignments = useMemo(() => {
    ...
    return assignments.filter(
      (item) => 
        item.course.toLowerCase().includes(course.title.toLowerCase()) ||
        course.title.toLowerCase().includes(item.course.toLowerCase())
    );
  }, [assignments, course]);

  const tabItems = useMemo(() => [
    { key: "about" as TabKey, label: "About" },
    { key: "assignments" as TabKey, label: `Assignments (${courseAssignments.length})` },
    { key: "lessons" as TabKey, label: `Lessons (${course?.lessons?.length || 0})` },
  ], [courseAssignments.length, course?.lessons?.length]);
